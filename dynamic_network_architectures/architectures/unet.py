...
import torch
from dynamic_network_architectures.building_blocks.convnext_encoder import ConvNextEncoder
...
class ConvNextEncoderUNet(nn.Module):
    def __init__(self,
                 ...
                 num_classes: int,
                 n_conv_per_stage_decoder: Union[int, Tuple[int, ...], List[int]],
                 decoder_conv_bias: bool = True,
                 decoder_norm_op: Union[None, Type[nn.Module]] = None,
                 decoder_norm_op_kwargs: dict = None,
                 decoder_dropout_op: Union[None, Type[_DropoutNd]] = None,
                 decoder_dropout_op_kwargs: dict = None,
        ...
        self.decoder = UNetDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                   nonlin_first=False, norm_op=decoder_norm_op, norm_op_kwargs=decoder_norm_op_kwargs,
                                   dropout_op=decoder_dropout_op, dropout_op_kwargs=decoder_dropout_op_kwargs,
                                   nonlin=decoder_nonlin, nonlin_kwargs=decoder_nonlin_kwargs,
                                   conv_bias=decoder_conv_bias)
...
if __name__ == '__main__':
    ...
    model = ConvNextEncoderUNet(
        4, 6, [32, 64, 128, 256, 320, 320], nn.Conv3d, [3, 7, 7, 7, 7, 7], [1, 2, 2, 2, 2, 2], [2, 3, 3, 6, 6, 6],
        11, 2, True, nn.InstanceNorm3d, {}, nn.Identity, {}, nn.LeakyReLU, {'inplace': True}, 0, True
    )
    out = model(data)
    # if True:
    #     import hiddenlayer as hl
    ...
    #     g.save("network_architecture.pdf")
    #     del g
    print(data.shape, model.compute_conv_feature_map_size(data.shape[2:]))

    data = torch.rand((1, 4, 512, 512))

    model = ConvNextEncoderUNet(
        4, 7, [32, 64, 128, 256, 512, 512, 512], nn.Conv2d, [3, 7, 7, 7, 7, 7, 7], [1, 2, 2, 2, 2, 2, 2], [2, 3, 3, 6, 6, 6, 6],
        11, 2, True, nn.InstanceNorm3d, {}, nn.Identity, {}, nn.LeakyReLU, {'inplace': True}, 0, True
    )
    out = model(data)
    print(data.shape, model.compute_conv_feature_map_size(data.shape[2:]))
